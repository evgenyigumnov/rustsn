pub fn explore_files(path: &str, include_ext: &Vec<String>, exclude_path: &Vec<String>) -> Vec<String> {
    let mut files = Vec::new();
    let paths = std::fs::read_dir(path).unwrap();
    for path in paths {
        let path = path.unwrap().path();
        if path.is_dir() {
            files.append(&mut explore_files(path.to_str().unwrap(), include_ext, exclude_path));
        } else {
